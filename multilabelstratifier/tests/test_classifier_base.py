...
#     def get_multilabel_data_for_tests(self, sparsity_indicator, fixture_y, fixture_x):
#         feed_sparse = sparsity_indicator == "sparse"
#
#         if feed_sparse:
#             return [(sparse.csr_matrix(fixture_x), sparse.csr_matrix(fixture_y))]
#         else:
#             return [(np.matrix(fixture_x), np.matrix(fixture_y))]
#
...
#
#             scores = model_selection.cross_val_score(classifier, X, y=y, cv=cv, scoring="accuracy")
#
